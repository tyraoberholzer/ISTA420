...
select empid, max(orderdate) as datOfLastActivity from sales.orders group by empid;
...
select o.empid, max(o.orderdate), o.orderid, o.custid 
	...
	(select empid, max(orderdate) as dateOfLastActivity from sales.orders group by empid) as c
		...
		group by o.empid, o.orderid, o.custid;
...
ROW_NUMBER() OVER(ORDER BY orderdate, orderid) AS rownum
FROM Sales.Orders)
select orderid, orderdate, custid, empid, rownum from name 
...
drop view if exist Sales.VEmpOrders;
go
...
SELECT v1.empid, v1.orderyear, v1.totalqty,
(SELECT SUM(totalqty)
